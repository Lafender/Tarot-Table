...
all of the cards in the deck are called by their integer location in the array.
...
it will have its length checked and will be cleared when its length is equal to or greater than the newdeck variable
*/
var drawn = [newdeck]; // here newdeck can be replaced with any integer.

var test_interval; 


function showcard(x, img) {
	// x is the integer which selects where on the table the card will be placed
	// img is the integer that selects a card name from the cardlist array
	document.getElementById(cards[x]).src = "tarot/" + cardlist[img];
	// warning: there is no "cards" array in "deck.html" 
};

/*
the draw() funtion is called when a card image is clicked
findcard is passed to showcard(x) and is based on the id (from the html doc) of which card was clicked.
*/
...
/*
functions below this comment are debugging commands.
use them to check if all your card images are loading correctly
*/
